...
export default function ChatWidget() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end font-sans">
      {isOpen && (
        <div className="mb-4 w-[90vw] sm:w-[360px] h-[520px] max-h-[80vh] bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.12)] border border-stone-100 flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
          {/* Header */}
          <div className="px-5 py-4 border-b border-stone-50 flex justify-between items-center bg-white/80 backdrop-blur-md sticky top-0 z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold text-stone-800 text-sm">Assistant</h3>
                <p className="text-[10px] text-stone-400 font-medium tracking-wide uppercase">Portra CV</p>
              </div>
            </div>
            <button onClick={() => setIsOpen(false)} className="text-stone-400 hover:text-stone-600 p-2 rounded-full transition-colors">
              <X size={18} />
            </button>
          </div>

          {/* Message List */}
          <div className="flex-1 p-5 overflow-y-auto space-y-6 bg-white scrollbar-thin scrollbar-thumb-stone-100">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex flex-col ${msg.sender === 'user' ? 'items-end' : 'items-start'}`}>
                <div className={`
                            max-w-[85%] px-4 py-3 rounded-3xl text-[13px] leading-relaxed shadow-sm
                            ${msg.sender === 'user'
                    ? 'bg-stone-900 text-white rounded-br-none'
                    : msg.isError
                      ? 'bg-red-50 text-red-600 border border-red-100 rounded-bl-none'
                      : 'bg-stone-50 text-stone-600 border border-stone-100 rounded-bl-none'}
                        `}>
            ...
            {isTyping && (
              <div className="flex justify-start">
                <div className="bg-stone-50 px-4 py-3 rounded-3xl rounded-bl-none flex gap-1">
          ...
          <form onSubmit={handleSend} className="p-4 bg-white border-t border-stone-50">
            <div className="relative flex items-center group">
              <input
                ...
                className="w-full bg-stone-50 text-stone-800 placeholder:text-stone-400 text-sm rounded-full pl-5 pr-12 py-3.5 focus:outline-none focus:ring-1 focus:ring-stone-200 transition-all border-none"
              />
              <button
                type="submit"
                disabled={!input.trim() || isTyping}
                className="absolute right-2 p-2 bg-stone-900 hover:bg-stone-800 disabled:bg-stone-200 rounded-full text-white transition-all transform active:scale-95 flex items-center justify-center"
              >
